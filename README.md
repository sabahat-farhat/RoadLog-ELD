...
|---|---|---|
| `POST` | `/api/trips/` | Create a trip: `{current_location, pickup_location, dropoff_location, current_cycle_used}` (each location is `{label, lat, lon}`); returns the full computed trip |
